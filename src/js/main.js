...
function setSpecificArray(recipes) {
    recipes.forEach(r => {
        ...
        r.ingredients.forEach(i => {
            if (arrayOfIngredients.indexOf(i.ingredient) === -1) arrayOfIngredients.push(i.ingredient);
        });
    });
    setTags();
};
...
function setRecipesCardHtml(recipes) {
    ...
    recipes.forEach((r) => {
        recipesList.append(`
            <article class="col-md-4 recipe recipeId-${r.id}">
                <div class="card mb-4 box-shadow no-border">
                    <img class="card-img-top" src="public/img/bg-recipe.png" alt="Card image cap" />
                    <div class="card-body card-body-style">
                        <div class="d-flex justify-content-between">
                            <label class="card-title">${r.name}</label>
                            <div class="card-time"><img src="./public/img/clock-icon.png" class="icon clockIcon" /> <span>${r.time} min</span></div>
                        </div>
...
function toggleDropdownList(elementId) {
    $(`#${elementId}`).toggle();
    if ($(`#${elementId}`)[0].style.display == 'block') toggleIconDropdown(true, elementId);
    else toggleIconDropdown(false, elementId);
};

// Toggle icon of input
function toggleIconDropdown(bool, elementId) {
    // Icon is up if value is found
    if (bool) {
        $(`#icon-down-${elementId}`).css("display", "none");
        $(`#icon-up-${elementId}`).css("display", "inherit");
    } else {
        // Default icon is down
        $(`#icon-down-${elementId}`).css("display", "inherit");
        $(`#icon-up-${elementId}`).css("display", "none");
    }
};

// Show / Hide tag on dropdown list
function filterByDropdownText(inputElementId, dropdownListId) {
...
//Remove tag
function deleteTagById(elementId) {
    for (var i = 0; i < arrayOfTagId.length; i++) {
        if (arrayOfTagId[i] === elementId) {
            $(`.${elementId}`).remove();
...
$(".dropDownList").on("click", "li", function (event) {
    ...
    if (!arrayOfTagId.includes(event.target.id)) {
        ...
        if (delegateTarget == 'dropdownApplianceList') colorBtn = 'btnGreen';
        ...
        tagList.append(`
            <button type="button" class="btn btn-info btnDefault ${colorBtn} ${event.target.id}" 
                ...
                ${event.target.text} 
                <img src="./public/img/remove-icon.png" class="icon removeIcon" alt="Remove icon"/>
            </button>
        `);
    }
    filterRecipesByTags(arrayOfTagValue);
